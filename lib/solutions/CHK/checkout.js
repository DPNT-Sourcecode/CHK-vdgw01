...
const applyGroupOffers = (skuCount, currentTotal) => {
    ...
    while(totalIncludedSkuCount >= groupOffers.specialOfferAmount) {
        totalIncludedSkuCount -= groupOffers.specialOfferAmount
        for(let i = 0; i < groupOffers.specialOfferAmount; i++) {
            sortedSkuPrices.every((value) => {
                if(skuCount[value[0]] != 0) {
                    skuCount[value[0]] -= 1
                    return false
                }
                return true
        ...
        currentTotal += groupOffers.specialOfferPrice
    }
